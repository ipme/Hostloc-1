...
def check_login_status(s: req_Session, number_c: int) -> bool:
    ...
    if len(test_title) != 0:  # 确保正则匹配到了内容，防止出现数组索引越界的情况
        if test_title[0] != "个人资料 -  全球主机交流论坛 -  Powered by Discuz!":
            ...
            content = content + "登录失败！\n"
            return False
        else:
            print("第", number_c, "个帐户登录成功！")
            content = content + "登录成功！\n"
            return True
    else:
        print("无法在用户设置页面找到标题，该页面存在错误或被防 CC 机制拦截！")
        content = content + "无法在用户设置页面找到标题，该页面存在错误或被防 CC 机制拦截！" + "\n"
        return False
...
def print_current_points(s: req_Session):
    ...
    if len(points) != 0:  # 确保正则匹配到了内容，防止出现数组索引越界的情况
        ...
        content = content + "帐户当前积分：" + str(points[0]) + "\n"
    else:
        print("无法获取帐户积分，可能页面存在错误或者未登录！")
        content = content + "无法获取帐户积分，可能页面存在错误或者未登录！" + "\n"
    time.sleep(6)
...
def print_my_ip():
    ...
    try:
        ...
        content = content + "当前使用 ip 地址：" + res.text + "\n"
    except Exception as e:
...
def sendTgBot(content):
    ...
    for retry_ in range(4):  
        posttext=requests.post(r'https://api.telegram.org/bot'+tg_bot[1]+r'/sendMessage',headers=headers,data=json.dumps(data))
...
if __name__ == "__main__":
    ...
    if not username or not password:
        ...
        content = content + "未检测到用户名或密码，请检查环境变量是否设置正确！\n"
    elif len(user_list) != len(passwd_list):
        print("用户名与密码个数不匹配，请检查环境变量设置是否错漏！")
        content = content + "用户名与密码个数不匹配，请检查环境变量设置是否错漏！\n"
    else:
        print_my_ip()
        print("共检测到", len(user_list), "个帐户，开始获取积分")
        print("*" * 30)
        content = content + "共检测到" + str(len(user_list)) + "个帐户，开始获取积分\n" + "-" * 30 + "\n"

        # 依次登录帐户获取积分，出现错误时不中断程序继续尝试下一个帐户
        for i in range(len(user_list)):
            try:
                s = login(user_list[i], passwd_list[i])
                content = content + "账户 " + str(user_list[i]) + " 进行积分获取，"
                get_points(s, i + 1)
                print("*" * 30)
                content = content + "-" * 30 + "\n"
            except Exception as e:
